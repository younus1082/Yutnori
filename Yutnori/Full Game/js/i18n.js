...
const STRINGS = {
  en: {
    ...
    scoreHome: '{finished} / {total} home',
    throwPrompt: 'Throw the sticks',
    movesToPlace: '{n} to place',
...
// Initial locale is carried over from the entrance page via ?lang= (and
// persisted in localStorage), so switching languages on the home screen
// keeps the game in the same language. Falls back to English.
function readInitialLocale() {
  ...
  try {
    const s = localStorage.getItem('yutnori.lang');
...
export function setLocale(next) {
  ...
  locale = next;
  try { localStorage.setItem('yutnori.lang', next); } catch (e) { /* ignore */ }
